...
DIR <- '/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/COMPASS_junctions/'

last_name_first_author <- "Roy" 
SRR_PREFIX <- "SRR91" 
...
last_name_first_author <- 'Aslanzadeh_Roy_upf1_replicate_for_aligner_comparison'
sample_key <- read_tsv('/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/keyfiles/combined_sample_key.tsv')

annotated_introns_filename = '/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/keyfiles/S_cerevisiae_all_introns.txt'
...
# format of annotated_introns_file:
# chrom	start	end	strand	type	Name	intron_type
# chrI	87388	87500	+	intron	YAL030W_intron	spliceosomal_intron
# chrI	139188	139218	+	intron	tP(UGG)A_intron	tRNA_intron

junction_df <- combined_junctions %>% mutate(start = adj_start + 1, end = adj_stop + 1, strand = RNA_strand) %>% 
  left_join(annotated_introns) %>% 
  mutate(seqnames = chrom,
         intron_size = adj_stop - adj_start + 1,
         ...
         adj_junc_US = (substr(US_5SS_10nt, 10, 10) == substr(US_3SS_10nt, 10, 10) ),
         adj_junc_DS = (substr(DS_5SS_10nt, 1, 1) == substr(DS_3SS_10nt, 1, 1) ) ,
  ...
  select(-adj_start, -adj_stop, -five_SS, -three_SS) %>%
  left_join(sample_key)

junction_df %>% write_tsv( paste0(DIR, last_name_first_author, "_COMPASS_splice_junctions_junction_df.tsv")  )

# last_name_first_author <- 'Aslanzadeh_Talkish_Roy_combined'
junction_df <- read_tsv( paste0(DIR, last_name_first_author, "_COMPASS_splice_junctions_junction_df.tsv") )

junction_df_summarised <- junction_df
filtered_junction_df_summarised <- junction_df_summarised %>% filter(chrom != 'chrmt', intron_size > 40, intron_size < 2000, 
                                                                    ...
                                                                    # chrII 276125 276179
                                                                    # CT      AC  junctions appear to be artifacts on strand switching of abundant GT-AG junctions due to incomplete dUTP digestion.
...
filtered_junction_df_summarised
...
filtered_junction_df_summarised  %>% 
  filter(aligner == "COMPASS", COMPASS_counts > 0)

sample_to_plot = "SRR9130288"

filtered_junction_df_summarised %>% group_by(sample_name) %>% tally()

filtered_junction_df_read_counts_only <- filtered_junction_df_summarised %>% 
  filter(sample_name == sample_to_plot, aligner == "COMPASS", !annotated_junction) %>%
  select(intron_ID, COMPASS_counts, bbmap_counts, STAR_default_counts, 
  STAR_noncanonical_counts, HISAT2_default_counts, HISAT2_noncanonical_counts,
  MAGIC_BLAST_counts, GSNAP_counts)

filtered_junction_binary_df <- filtered_junction_df_read_counts_only %>% mutate_if(is.numeric, ~1 * (. > 0))

pdf(file = paste0('/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/plots/', sample_to_plot, '_unannotated_junctions_passing_filter_by_aligner_false_positives_not_shared_with_other_aligners.pdf'), 
    width = 12, # The width of the plot in inches
    height = 8, # The height of the plot in inches
    onefile=FALSE )
upset( as.data.frame(filtered_junction_binary_df), nsets = 7, nintersects = 20,
      sets = c("bbmap_counts", "STAR_default_counts", "STAR_noncanonical_counts", 
      "HISAT2_default_counts", "HISAT2_noncanonical_counts", "MAGIC_BLAST_counts", "GSNAP_counts"), 
      order.by="freq", matrix.color="#ABB065", point.size=5, sets.bar.color=brewer.pal(n = 7, name = "Set1"))
dev.off()

filtered_junction_df_read_counts_only <- junction_df %>% 
  filter(sample_name == sample_to_plot, aligner == "COMPASS", !annotated_junction) %>%
  select(intron_ID, COMPASS_counts, bbmap_counts, STAR_default_counts, 
  STAR_noncanonical_counts, HISAT2_default_counts, HISAT2_noncanonical_counts,
  MAGIC_BLAST_counts, GSNAP_counts)
...
pdf(file = paste0('/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/plots/', sample_to_plot, '_unannotated_junctions_by_aligner_false_positives_not_shared_with_other_aligners.pdf'), 
    width = 12, # The width of the plot in inches
    height = 8, # The height of the plot in inches
    onefile=FALSE )
upset( as.data.frame(filtered_junction_binary_df), nsets = 7, nintersects = 20,
...
filtered_junction_df_read_counts_only <- filtered_junction_df_summarised %>% 
  filter(sample_name == sample_to_plot, !annotated_junction) %>%
  select(intron_ID, aligner, COMPASS_counts)

intron_ID_dedup <- filtered_junction_df_read_counts_only %>% group_by(intron_ID, aligner) %>% summarize(total_counts = sum(COMPASS_counts)) %>% 
mutate(aligner = if_else(aligner == "COMPASS", aligner, substr(aligner, 2, nchar(aligner)) ))
intron_ID_dedup

filtered_junction_binary_df <- intron_ID_dedup %>% spread(key = aligner, value = total_counts, fill = 0)

filtered_junction_binary_df <- filtered_junction_binary_df %>% select(intron_ID, COMPASS, bbmap, STAR_default, STAR_noncanonical, 
      HISAT2_default, HISAT2_noncanonical, MAGIC_BLAST, GSNAP)

filtered_junction_binary_df <- filtered_junction_binary_df %>% mutate_if(is.numeric, ~1 * (. > 0))

pdf(file = paste0('/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/plots/', sample_to_plot, '_unannotated_junctions_passing_filter_by_aligner_false_positives_not_shared_with_COMPASS.pdf'), 
    width = 12, # The width of the plot in inches
    height = 8, # The height of the plot in inches
    onefile=FALSE )
upset( as.data.frame(filtered_junction_binary_df), nsets = 8, nintersects = 20,
      sets = c("COMPASS", "bbmap", "STAR_default", "STAR_noncanonical", 
      "HISAT2_default", "HISAT2_noncanonical", "MAGIC_BLAST", "GSNAP"), 
      order.by="freq", matrix.color="#ABB065", point.size=5, sets.bar.color=brewer.pal(n = 8, name = "Set1"))
dev.off()
...
pdf(file = paste0('/oak/stanford/groups/larsms/kevinroy/processed_data/COMPASS/plots/', sample_to_plot, '_unannotated_all_junctions_by_aligner_false_positives_not_shared_with_COMPASS.pdf'), 
    width = 12, # The width of the plot in inches
    height = 8, # The height of the plot in inches
    onefile=FALSE )
upset( as.data.frame(filtered_junction_binary_df), nsets = 8, nintersects = 20,
...
## 
junction_df_filtered <- junction_df %>% filter(intron_ID %in% filtered_junction_df_summarised$intron_ID)
...
junction_df_summarised %>% filter(intron_size >= 2000) # 43,450 
...
previous_SJ <-readxl::read_excel('/Volumes/GoogleDrive/My Drive/Scripts/COMPASS/splicing_datasets/previous_studies_SJ.xlsx') %>% 
...
combined_tbl %>% filter(adj_5SS != 'GT', FAnS > 0.1 | is.na(FAnS), !in_previous_datasets, min_score_unannotated_intron_junctions_disagree/COMPASS_counts < .1) %>% 
  ...
  select(intron_ID, adj_5SS, adj_3SS, bbmap_counts, HISAT2_noncanonical_counts, STAR_default_counts, FAnS,
...
ggplot(splice_site_tally %>%
         filter(adj_5SS != 'GT', # !adj_junc_US, !adj_junc_DS,
...
filtered_junction_df_summarised %>% filter(!adj_junc_US, !adj_junc_DS) %>% 
...
filtered_junction_df_summarised %>% ungroup() %>% group_by(five_SS_2nt, adj_junc_US | adj_junc_DS) %>% tally() %>% print(n =100)

filtered_junction_df_summarised %>% ungroup() %>% filter(five_SS_2nt == 'CA', !adj_junc_US, !adj_junc_DS) %>% 
...
filtered_junction_df_summarised %>% ungroup() %>% filter(five_SS_2nt == 'AT', !adj_junc_US, !adj_junc_DS,
...
UBC12 <- junction_df_longer %>% filter(adj_start == 744153, adj_stop >= 744228, adj_stop <= 744289)
...
p1 <- ggplot(filtered_junction_df_summarised_longer %>%
               filter(total_counts != 0, adj_5SS == 'GT', !annotated_junction, # !adj_junc_US, !adj_junc_DS,
                      # !str_detect(aligner, 'default') 
               ),  
...
p2 <- ggplot(filtered_junction_df_summarised_longer %>%
               filter(total_counts != 0, adj_5SS != 'GT',  # !adj_junc_US, !adj_junc_DS,
                      #  !str_detect(aligner, 'default') 
               ),  
             aes(x = forcats::fct_infreq(adj_5SS), fill = aligner)) +
  geom_bar(position = position_dodge(preserve = "single")) + theme_Publication() + 
...
p3 <- ggplot(filtered_junction_df_summarised_longer %>%
               filter(total_counts != 0, adj_3SS == 'AG', !annotated_junction, # !adj_junc_US, !adj_junc_DS,
                      #  !str_detect(aligner, 'default') 
               ),  
...
p4 <- ggplot(filtered_junction_df_summarised_longer %>%
               filter(total_counts != 0, adj_3SS != 'AG', # !adj_junc_US, !adj_junc_DS,
                      # !str_detect(aligner, 'default') 
               ),  
             aes(x = forcats::fct_infreq(adj_3SS), fill = aligner)) +
  geom_bar(position = position_dodge(preserve = "single")) + theme_Publication() +
